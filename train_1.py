...
###
# from config.config_1 import POLICY_CONFIG, TASK_CONFIG, TRAIN_CONFIG, USE_WANDB, DATA_LOADER_CONFIG # must import first
...
#         # training
#         policy.train()
#         epoch_train_dicts = []  # Store batch results for this epoch only
#         for batch_idx, data in enumerate(train_dataloader):
#             forward_dict = forward_pass(data, policy)
#             loss = forward_dict['loss']
#             loss.backward()
...
#         train_history.append(epoch_summary)  # Store only epoch-level stats
...
#             project="ACT_origin",
#             entity=None,  # Change to your wandb team name if needed
#             name=f"{task}_optimized_{train_cfg['seed']}",
#             config={
#                 "task": task,
#                 # Task config
#                 **{f"task_cfg/{k}": v for k, v in TASK_CONFIG.items()},
...
#                 # Policy config
#                 **{f"policy_cfg/{k}": v for k, v in POLICY_CONFIG.items()},
#                 **{f"policy_cfg/{k}": v for k, v in DATA_LOADER_CONFIG.items()},
#                 'remarks': 'Optimized: pre-loaded data + pre-computed transformations',
...
#     # load data (with pre-loading optimization)
#     train_dataloader, val_dataloader, stats, _ = load_data(
#         data_dir, 
#         batch_size_train=train_cfg['batch_size_train'], 
#         batch_size_val=train_cfg['batch_size_val'],
